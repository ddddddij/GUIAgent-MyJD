...
class HuaweiMate60DetailPresenter(
    private val repository: DataRepository
) : HuaweiMate60DetailContract.Presenter {
    
    private var view: HuaweiMate60DetailContract.View? = null
    private var currentProductDetail: ProductDetail? = null
    private val presenterScope = CoroutineScope(Dispatchers.Main + Job())
    
    override fun attachView(view: HuaweiMate60DetailContract.View) {
        this.view = view
    }
        ...
        presenterScope.launch {
            try {
                val productDetail = withContext(Dispatchers.IO) {
                    repository.getHuaweiMate60ProductDetail(productId)
                }
