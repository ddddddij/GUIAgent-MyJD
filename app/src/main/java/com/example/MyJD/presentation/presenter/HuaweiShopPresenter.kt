...
class HuaweiShopPresenter(
    ...
    override fun loadShopData() {
        presenterScope.launch {
            ...
            try {
                val data = repository.loadHuaweiShopPageData()
                shopData = data
                view?.showShopData(data)
                android.util.Log.d("HuaweiShopPresenter", "Loaded shop data: ${data.shopInfo.name}")
            } catch (e: Exception) {
                view?.showToast("店铺数据加载失败")
                android.util.Log.e("HuaweiShopPresenter", "Failed to load shop data", e)
            } finally {
    ...
    override fun onCategorySelected(categoryId: String) {
        ...
        android.util.Log.d("HuaweiShopPresenter", "Category selected: $categoryId")

        // 按要求：分类选择仅为UI展示，无实际过滤功能
    }

    override fun onProductClick(product: Product) {
        android.util.Log.d("HuaweiShopPresenter", "Product clicked: ${product.name}")
    ...
    override fun onAddToCartClick(product: Product) {
        try {
            ...
            android.util.Log.d("HuaweiShopPresenter", "Added to cart: ${product.name}")

        } catch (e: Exception) {
            view?.showToast("添加到购物车失败")
            android.util.Log.e("HuaweiShopPresenter", "Failed to add to cart", e)
        }
    }

    override fun onBackClick() {
        android.util.Log.d("HuaweiShopPresenter", "Back button clicked")
        view?.navigateBack()
    }
