package com.example.myjd.domain.model
...
data class AppState(
    ...
    val pendingPaymentOrders: List<Order>
        get() = orders.filter { it.status == com.example.myjd.domain.model.OrderStatus.PENDING_PAYMENT }
    
    val pendingShipmentOrders: List<Order>
        get() = orders.filter { it.status == com.example.myjd.domain.model.OrderStatus.PENDING_SHIPMENT }
    
    val pendingReceiptOrders: List<Order>
        get() = orders.filter { it.status == com.example.myjd.domain.model.OrderStatus.PENDING_RECEIPT }
    
    val pendingReviewOrders: List<Order>
        get() = orders.filter { it.status == com.example.myjd.domain.model.OrderStatus.PENDING_REVIEW }
    
    val completedOrders: List<Order>
        get() = orders.filter { it.status == com.example.myjd.domain.model.OrderStatus.COMPLETED }
}